...
//    }
    ...
    public String findMenuList(@PageableDefault Pageable pageable, Model model){
        ...
        /*Page 객체가 담고 있는 정보*/
        log.info("조회한 내용 목록 : {}", menuList.getContent());    //List<MenuDTO> 반환
    ...
    @GetMapping("/querymethod1")
    public void queryMethodPage1(){}
    ...
    @GetMapping("/contain")
    public String findByMenuNameContaining(@RequestParam String menuName, Model model){

        List<MenuDTO> menuList = menuService.findByMenuNameContaining(menuName);
        model.addAttribute("menuList", menuList);
        return "menu/containResult";
    }

    /* 이거 내가 해본 건데...*/
    @GetMapping("/submit")
    public String findByMenuCodeAndMenuName(@RequestParam Integer menuCode, String menuName, Model model){
        MenuDTO menu = menuService.findByMenuCodeAndMenuName(menuCode, menuName);
        model.addAttribute("menu", menu);
        return "menu/submitResult";
    }


    @GetMapping("/find")
    public String findByMenuCodeOrMenuName(@RequestParam Integer menuCode, String menuName, Model model){
        MenuDTO menu = menuService.findByMenuCodeOrMenuName(menuCode, menuName);
        model.addAttribute("menu", menu);
        return "menu/findResult";
    }
